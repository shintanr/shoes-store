...
<body>
    ...
    <div class="row">
        <div class="col-1">
            <a type="button" class="btn btn-warning" href="/bike">Refresh</a>
        </div>
        <div class="col-1">
            <a type="button" class="btn btn-info" href="/createbike">Tambah</a>
        </div>
        <div class="col-1">
            <a type="button" class="btn btn-danger" href="/trashbike">Terhapus</a>
        </div>
        <div class="col">
            <!-- Dropdown Start -->
            <form action="/bike" method="get" class="form-inline">
                <div class="input-group">
                    ...
                    <select name="sort_by" id="sort_by" class="form-select" onchange="this.form.submit()">
                        <option value="" {{ empty(request('sort_by')) ? 'selected' : '' }}>-- Pilih --</option>
                        <option value="alphabet" {{ request('sort_by') == 'alphabet' ? 'selected' : '' }}>A-Z (Nama sepatu)</option>
                        <option value="reversed" {{ request('sort_by') == 'reversed' ? 'selected' : '' }}>Z-A (Nama sepatu)</option>
                    </select>
                </div>
            </form>
            <!-- Dropdown End -->
    ...
    <tbody>
      @foreach ($data as $index => $row)
          <tr>
              ...
              <td>{{ $row->harga }}</td>
              <td>
                  <a href="{{ route('editBikePage', ['id' => $row->id_sepatu]) }}" class="btn btn-primary">Edit</a>
  
                  <form action="{{ route('deleteBike', ['id' => $row->id_sepatu]) }}" method="post" class="d-inline">
                      @csrf
                      @method('put')
